 ...
 */
package ai.lamin.plugin.instance

import ai.lamin.plugin.LaminConfig
import ai.lamin.plugin.hub.LaminHub
import ai.lamin.plugin.hub.LaminHubConfigResolver
import ai.lamin.plugin.instance.Instance
import ai.lamin.plugin.instance.InstanceSettings
...
class InstanceStagingTest extends Specification {
    ...
    def "should connect to staging instance and get basic information"() {
        when:
    ...
    @IgnoreIf({ !env.LAMIN_STAGING_API_KEY })
    def "should fetch instance statistics from staging API"() {
        when:
    ...
    @IgnoreIf({ !env.LAMIN_STAGING_API_KEY })
    def "should get non-empty tables from staging instance"() {
        when:
