...
/**
 * Unit test for Lamin DSL
 *
 * @author : jorge <jorge.aguilera@seqera.io>
 */
@Timeout(10)
class LaminDslTest extends Dsl2Spec{
    ...
    def 'should perform a hi and create a channel' () {
        when:
        def SCRIPT = '''
            include {reverse} from 'plugin/nf-lamin'
            channel.reverse('hi!') 
            '''
        and:
        def result = new MockScriptRunner([lamin:[prefix:'>>']]).setScript(SCRIPT).execute()
        then:
    ...
    def 'should store a goodbye' () {
        when:
        def SCRIPT = '''
            include {goodbye} from 'plugin/nf-lamin'
            channel
                .of('folks')
                .goodbye() 
            '''
    ...
    def 'can use an imported function' () {
        when:
        def SCRIPT = '''
            include {randomString} from 'plugin/nf-lamin'
            channel
                .of( randomString(20) )                
            '''
        and:
